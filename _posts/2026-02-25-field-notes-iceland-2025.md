...
To understand the Icelandic interior, one must learn to read the language of lava. The landscape is dominated by two common surface forms of basaltic flows: _hella-hraun_ (smooth, ropey pāhoehoe) and _apal-hraun_ (jagged, blocky ʻaʻā). Walking across an ancient _apal-hraun_ field is a lesson in tactile geography. Every surface is razor-sharp, encrusted with a brittle layer of grey-green wool-moss (_Racomitrium lanuginosum_) that takes centuries to establish itself over the black glass. The contrast is conceptually striking—the softest, most fragile biological pioneer clinging to the hardest, most violent geological foundation.
...
{% include video.html
   ...
   caption="The southern coast in motion — surf retreating across black volcanic sand at Reynisfjara, Iceland."
   id="fig-coast-video"
%}
...
## Sonic Field Notes: The Sound of Black Pebbles

Images and video capture Iceland’s visible drama, but the coast is also an acoustic landscape. At Djúpalónssandur, the ocean is not only heard as a distant roar; it becomes a close, textured rhythm. Water breaks against the shore, then recedes through black pebbles in a frothy pattern of friction, pull, and return. The sound gives the landscape a physical scale that still images cannot fully carry.

The audio sample in {% include ref.html id="fig-djupalonssandur-audio" cref="true" %} demonstrates the `audio.html` include using a local Opus audio file. This keeps the media lightweight while still allowing the post to include place-based sound as part of the field-note experience.
...
This sonic layer changes how the surrounding images are read. The basalt columns in {% include ref.html id="fig-basalt" cref="true" %} become less static when paired with the repeated movement of water across stone. The landscape is no longer only a composition of cliffs, sand, ice, and light; it becomes a timed event, shaped by impact, retreat, and recurrence.

## Testing Proportions and Spatial Dynamics

Reusable media components prove their value when they preserve a coherent visual rhythm across different providers and screen sizes. After the YouTube embed above establishes the template's handling of a wide coastal horizon, the Vimeo capture in {% include ref.html id="fig-canyon-video" cref="true" %} demonstrates that the same `video.html` include can embed a second platform while maintaining the same 16:9 structure, caption treatment, and responsive figure spacing.

{% include video.html
   id="fig-canyon-video"
   provider="vimeo"
   embed="https://player.vimeo.com/video/715601839?h=889d734493"
   ratio="16-9"
   title="White waves receding across black sand along the Icelandic shoreline"
   caption="A top-down shoreline study: white waves thinning across Iceland's black sand in a high-contrast coastal frame."
%}

In this footage, the emphasis remains on motion and contrast rather than on geological depth. From a top-down vantage, the viewer watches white surf dissolve into thin retreating lines across volcanic black sand, turning the shoreline into a study in texture, repetition, and tonal opposition. The key test here is structural consistency rather than a new aspect ratio: whether this page is viewed on an ultra-wide desktop monitor or a handheld mobile device, the responsive 16:9 container keeps the Vimeo embed aligned with the surrounding typography, preserving proportional balance without introducing layout drift between providers.

## Local Motion: Strokkur as a Self-Hosted Video

Third-party embeds are useful when a video already lives on YouTube or Vimeo, but a portfolio template should also handle local video assets. A self-hosted video is especially useful for short project clips, field recordings, animation tests, and research demos where the media belongs directly to the site archive.

The local video in {% include ref.html id="fig-strokkur-local-video" cref="true" %} tests the self-hosted mode of the `video.html` include. Unlike the YouTube and Vimeo examples above, this file is served directly from the site’s own assets folder while preserving the same responsive 16:9 frame, caption styling, and figure numbering.

{% include video.html
   id="fig-strokkur-local-video"
   src="
     assets/videos/posts/geysir-iceland-2023-erupting-geyser-strokkur.webm | video/webm
   "
   poster="assets/images/posts/geysir-iceland-2023-erupting-geyser-strokkur.webp"
   ratio="16-9"
   preload="metadata"
   title="Self-hosted video of Strokkur geyser erupting in Iceland"
   caption="Strokkur erupting in the Geysir geothermal area, Iceland. Source video: [Geysir, Iceland, 2023 - Erupting geyser Strokkur](https://www.vecteezy.com/video/23212018-geysir-iceland-2023-erupting-geyser-strokkur-strokkur-is-part-of-geothermal-area) by EvaL MiKo on Vecteezy, free video with attribution required. Converted from the original h264 source to WebM VP9 with Opus audio for lightweight GitHub Pages delivery."
%}

This test is intentionally different from the embedded videos above. The browser now renders a native `<video>` element with local playback controls, a poster frame, metadata preloading, and fallback text. That makes it a useful check for whether the template handles self-hosted media with the same visual rhythm as externally embedded video.
...
| `video.html`  | {% include ref.html id="fig-coast-video" %}  | 16:9 YouTube embed    | Expansive coastal horizons           |
| `video.html`  | {% include ref.html id="fig-canyon-video" %} | 16:9 Vimeo embed      | Provider parity and responsive depth |
| `video.html`  | {% include ref.html id="fig-strokkur-local-video" %} | 16:9 local WebM video with poster | Self-hosted playback, poster frame, native controls, and local asset delivery |
| `audio.html`  | {% include ref.html id="fig-djupalonssandur-audio" %} | Local Opus audio file | Ambient field recording and place-based sound |
